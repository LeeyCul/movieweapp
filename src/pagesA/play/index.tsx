...
import { View, Video, Button, Text, Ad } from '@tarojs/components';
  ...
  return (
    <View className="play_conainer">
            ...
            {rateArr.map((item, key) => {
              return (
                <View
                  className="rate_box"
                  key={item.name}
                  onClick={() => handleRateClick(item.rate, key)}
                >
                  <AtIcon
                    data-rate={1}
                    value="lightning-bolt"
                    size="18"
                    color={rate === key ? '#f00' : '#49b849'}
                  />
      ...
      <View className="adconainer">
        <Ad
          unitId="adunit-928560fe98892412"
          ad-type="video"
          ad-theme="white"
        ></Ad>
      </View>
      ...
      {status && status.play_show && isShowMore && (
        <PlayList title="选集" data={data && data.video_url} />
      )}
    </View>
  );
}
